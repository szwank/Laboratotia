...
feedbackDelays = 1:2;
hiddenLayerSize = 10;
% net = feedforwardnet([20,20,20,20,20],trainFcn);
net = narxnet(inputDelays,feedbackDelays,hiddenLayerSize,'open',trainFcn);
% net = layrecnet(feedbackDelays,hiddenLayerSize,trainFcn);
% net = closeloop(net);
...
% Moje ustawienia
 net.trainParam.max_fail = 500;      % Maximum validation failures
...
% gpu1 = gpuDevice(1)
net.sampleTime=0.1;     
% netg = configure(net,X,T);  % Configure with MATLAB arrays
% netg = init(netg);
% Xg = nndata2gpu(X');
% Tg = nndata2gpu(T);
% netg = train(netg,X,T,'useGPU','only','showResources','yes');    % Execute on GPU with NNET formatted gpuArrays
[x,xi,ai,t] = preparets(net,X,{},T);
% netg = closeloop(netg)
net = train(net,x,t,xi,ai,'CheckpointFile','MyCheckpoint.mat');
...
netc.divideFcn = 'divideblock';  
 ...
 netc.divideParam.trainRatio = 0.7;
 nec.divideParam.valRatio = 0.3;
 netc.divideParam.testRatio = 0;
  netc.trainParam.max_fail = 500;      % Maximum validation failures
 netc.trainParam.lr = 0.001;         % Learning rate
% 
%  netc.trainParam.epochs = 10^20;
% view(netc)

function [ X_PCA, X_PCAL_1, X_PCAL_pro, wektor_wartosci_wlasnych, macierz_wektorow_wlasnych ] = stworzPCA( dane_uczace, dane_testowe, procent_redukcij, procent_T2, procent_SPE )
...
hold on
rysuj_granice(SPE_limit, length(SPE));
title('SPE')
grid on
...
hold on
rysuj_granice(SPE_limit_L_1, length(SPE_L_1));
...
hold on
rysuj_granice(SPE_limit_L_pro, length(SPE_L_pro));
...
hold on
rysuj_granice(T2_limit, length(T2));
title('T2')
grid on
...
hold on
rysuj_granice(T2_limit_L_1, length(T2_L_1));
grid on
...
hold on
rysuj_granice(T2_limit_L_pro, length(T2_L_pro));
grid on
%{
...
title('WskaŸnik SPE- dane testowe, nie zredukowany')
legend('SPE- dane testowe','SPE- dane ucz¹ce')
grid on

subplot(2,2,2)
plot(length(SPE_L_pro) + 1:length(SPE_TL) + length(SPE_L_pro), SPE_TL, 'r*', 1 : length(SPE_L_pro) , SPE_L_pro, 'b*' )
rysuj_granice(SPE_limit_L_pro, (length(SPE_TL) + length(SPE_L_pro)));
title('WskaŸnik SPE- dane testowe, zredukowany')
legend('SPE- dane testowe','SPE- dane ucz¹ce')
grid on

subplot(2,2,3)
plot(length(T2) + 1: length(T2_T) + length(T2), T2_T, 'r*', 1 : length(T2), T2, 'b*')
rysuj_granice(T2_limit, (length(T2_T) + length(T2)));
title('WskaŸnik T2- dane testowe, nie zredukowany')
legend('T2- dane testowe','T2- dane ucz¹ce')
grid on

subplot(2,2,4)
plot(length(T2_L_pro) + 1: length(T2_TL) + length(T2_L_pro), T2_TL, 'r*', 1 : length(T2_L_pro), T2_L_pro, 'b*')
rysuj_granice(T2_limit_L_pro, (length(T2_TL) + length(T2_L_pro)));
title('WskaŸnik T2- dane testowe, zredukowany')
legend('T2- dane testowe','T2- dane ucz¹ce')
grid on
